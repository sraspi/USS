...
filename = ("/home/pi/data/Entfernung.txt")
logfile = ("/home/pi/data/logfile.txt")
...
fobj_out.write('\n' +  "Reboot " +  Datum + " ON.py: 3,3L/cm" + "16<D<11;   Dm>42 IBC-leer , write F(mm/min) n=116 t=20min"+ '\n' + '\n')
fobj_out.close()
...
fobj_out.write("---------------------------------------------" + '\n' + Datum + " started " + " n=: " + str(d) + ": ON-started" +" CPU_temp: " + str(round(cpu.temperature,1)) + "C"  + '\n' + '\n' )
fobj_out.close()
time.sleep(2)
    ...
    while True:
        ...
        if z > 116: # bereinigter Mittelwert aus 116*50 US-Messungen wird gespeichert
            ...
            if Dm > 40 and mail: #  ibc leer 
                  ...
                  cpu = CPUTemperature()
                  fobj_out = open(filename,"a")
                  fobj_out.write(Datum + " n=: " + str(d) +  ": IBC leer!!!  " + "Distanz: " +  str(round(mw,1)) + "  CPU_temp: " + str(cpu.temperature) + "C"  + " F:(mm/min): "+ str(round(F,2)) + '\n' + '\n' )
                  fobj_out.close()
                  mail = False

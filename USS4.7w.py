...
def c_write():
    ...
    for x in range (0, 24):
        if t_mail  ==  e[x] and v[x]:
            ...
            f.write(str(t_mail))
            f.close()
            if t_mail == 5: #from USS
                subprocess.call("/home/pi/US-Sensor/USmail.sh")
            if t_mail == 6: #from USS
                subprocess.call("/home/pi/US-Sensor/USmail.sh")
            if t_mail == 7: #from USS
                subprocess.call("/home/pi/US-Sensor/USmail.sh")
            if t_mail == 8: #from USS
                subprocess.call("/home/pi/US-Sensor/USmail.sh")
            if t_mail == 14: #from USS
                subprocess.call("/home/pi/US-Sensor/USmail.sh")
            if t_mail == 16: #from USS
                subprocess.call("/home/pi/US-Sensor/USmail.sh")
...
print("USS4.7w.py started")
print()
Datum = time.strftime("%Y-%m-%d %H:%M:%S")

fobj_out = open(logfile,"a")
fobj_out.write('\n' +  "Reboot " +  Datum + " USS4.7w.py started" + "16<D<11; Dm>55 and write F,n=116 t=20min"+ '\n' + '\n')
fobj_out.close()

print("Ventil winter_ON")
GPIO.output(22, GPIO.HIGH)
cpu = CPUTemperature()
fobj_out = open(filename,"a")
fobj_out.write("---------------------------------------------" + '\n' + Datum + " started " + " n=: " + str(d) + ": Ventil winter_ON!!!  "  + " CPU_temp: " + str(round(cpu.temperature,1)) + "C"  + '\n')
fobj_out.close()
time.sleep(2)
    ...
    while True:
        ...
        if z > 116: # bereinigter Mittelwert aus 116*50 US-Messungen wird gespeichert
            ...
            if R_on and Dm > 16: #wenn Dm > 16cm und R_on (also vorher auf OFF) dann Ventil ON
                ...
                cpu = CPUTemperature()
                fobj_out = open(filename,"a")
                fobj_out.write(Datum + " n=: " + str(d) + ": Ventil winter_OFF!!!  " + "Distanz: " + str(round(mw,1)) + " CPU_temp: " + str(round(cpu.temperature, 1)) + "C" + " F: " + str(round(F,2)) + '\n')
                fobj_out.close()
                #subprocess.call("/home/pi/US-Sensor/mail_on.sh")
                R_on = False
                R_off = True
